...
const SingleExercise = (props) => {
    ...
    useEffect(() => {
        async function filExercise() {
            try {
                ...
                const fillExer = await axios.get(`${apiKeys}/getExerciseSingle/${JSON.parse(localStorage.getItem('profile')).account}`)
                const exerciseMap = await axios.get(`${apiKeys}/getExerciseSingle/${JSON.parse(localStorage.getItem('profile')).account}/${topicParam}/${id}`)
                console.log(exerciseMap);
                setExprs(exerciseMap.data)
                const filterData = fillExer.data.exerciseSingle[topicParam].filter(item => {
    ...
    const handleMark = async (e, i) => {
        const getMark = await axios.get(`${apiKeys}/getArrExercisesSingle/${encodeURIComponent(JSON.parse(localStorage.getItem('profile')).account)}/${encodeURIComponent(e)}`)
    ...
    const handleFormula = async (e, i) => {
        ...
        const getFomula = await axios.get(`${apiKeys}/getArrExercisesSingle/${encodeURIComponent(JSON.parse(localStorage.getItem('profile')).account)}/${encodeURIComponent(e)}`)
