...
const DeleteSingleTopic = (props) => {
    ...
    useEffect(() => {
        async function indexs() {
            if (dirName, IB, nameTopic) {
                console.log('sucsss');
                const data = await axios.get(`${apiKeys}/getDeleteSingleExercise/${IB}/${dirName}/${nameTopic}`)
        ...
        return ({
            ...
            buttonDel: <button onClick={async () => {
                ...
                if (loopData) {
                    ...
                    try {
                        const delItem = await axios.delete(`${apiKeys}/deleteSingleTopic?fomula=${dirformula}&mark=${dirMark}&question=${question}&id=${id}&IB=${IB}&nameTopic=${nameTopic}&dirName=${dirName}`)
    ...
    return (
        <>
            ...
            <BasicSelect setActiveDraw={setActiveDraw} valueButton={'Chọn topic này'} name={dirMap && dirMap} handleClick={handleOption} label={"Chọn học sinh muốn xóa chủ đề"} activeDraw={activeDraw} />
...
export default connect(mapStateToProps, mapDispatchToProps)(DeleteSingleTopic)
