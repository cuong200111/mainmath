...
export default function BasicSelect(props) {
    ...
    const handleChange2 = async (event) => {
        ...
        if (nameDir.length > 0) {
            ...
            const listTopic = await axios.get(`${apiKeys}/getDirTopicSingle/${nameDir}/${value}`)
    ...
    return (
        <Box textAlign={"center"} display={Boolean(activeDraw) ? `block` : `flex`}
            alignItems="center" justifyContent="center" flexDirection="column" height={Boolean(activeDraw) ? `100px` : `100vh`}>
            ...
            <FormControl style={{ width: "50%", margin: "50px",display:activeDraw?`none`:`flex` }}>
            ...
            <button style={{display:activeDraw?`none`:`block`}} onClick={() => {
